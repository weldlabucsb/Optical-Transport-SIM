function [times,psies] = smooth_transport_simulation_ode45(distance,duration,omega,numstates)
...
%smooth_ramp
accel_smooth = @(t) 600.*((t<(duration/4)).*(distance/duration.^2).*(-(7040/9).*(t/duration).^3+320.*(t/duration).^2)+...
...
%again, the 600 is the factor due to the contribution of the quantum system
...
% linear_ramp:
H_t =@(t) omega.*(H_0 + accel_smooth(t).*(X));
